...
from constants.simulation_results.november_eighth import november_eighth
from constants.teams import TEAMS
from external_apis.cf_data import CFData
from ratings.inputs.data.team_ratings.week_ten import TEAM_RATINGS as TR_WEEK_TEN
...
def team_ratings():
    return json.jsonify(TR_WEEK_TEN)
...
def teams():
    try:
        adj_teams_object = {
            team: dict(
                power_rtgs=TR_WEEK_TEN[team],
                avg_power_rtg=round(sum([rtg for rtg in TR_WEEK_TEN[team].values()]) / len(TR_WEEK_TEN[team]), 1),
                **team_obj,
            )
...
def simulate():
    return json.jsonify(november_eighth)
